...
def feature_generate_tsfresh():
    train_df = pd.read_csv('../feature/train.csv')
    X_train = train_df.drop(columns=['type'])
    y_train = train_df['type']

    test_df = pd.read_csv('../feature/testB.csv')
...
if __name__ == "__main__":
    ...
    pd.DataFrame(result, index=range(9000, 11000)).to_csv('../submit/result.csv', header=None)